...
export function DailyRhythm({ week, date, tomorrowTopic, locale, t }: {
  week: Day[];
  date: string;
  tomorrowTopic: Topic;
  locale: Locale;
    ...
    <section className="discovery-week" aria-label={t('discoveryWeek')}>
      ...
      <ol className="week-stamps">
        {week.map(day => {
          ...
          return <li key={day.date} className={(day.completed ? 'played ' : '') + (day.date === date ? 'is-today' : '')}>
            <time dateTime={day.date} aria-current={day.date === date ? 'date' : undefined} aria-label={label + ' · ' + t(day.completed ? 'dailyDoneState' : 'puzzleOpen')}>
              <span className="week-day" aria-hidden="true">{value.toLocaleDateString(locale, { weekday: 'short', timeZone: 'UTC' })}</span>
              <span className="week-stamp" aria-hidden="true">{day.completed ? <Check size={17} strokeWidth={2.5}/> : value.getUTCDate()}</span>
            </time>
          </li>;
        })}
      </ol>
