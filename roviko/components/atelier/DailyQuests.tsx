...
/** Progress kept in this browser: the mystery answer and the crowns. One string, so React can compare snapshots. */
function localSnapshot(date: string) {
  ...
  return [+mystery, crowns.length, +crowns.includes(date)].join('|');
}
...
export function useQuests(date: string, sessions: Session[]) {
  const snapshot = useSyncExternalStore(subscribe, () => localSnapshot(date), () => '0|0|0');
  const [mystery, crowns, crowned] = snapshot.split('|').map(Number);
  const quests = questsFor(date, { completedModes: sessions.filter(s => s.completed).map(s => s.mode), mysteryPlayed: !!mystery });
...
export function DailyQuests({ date, sessions, t, compact = false, art = false, onPick }: { date: string; sessions: Session[]; t: (k: string) => string; compact?: boolean; art?: boolean; onPick?: (quest: Quest) => void }) {
  ...
  const label = (q: Quest) => q.kind === 'mode' ? t('questPlayMode').replace('{game}', t(dailyTitleKey(q.mode!))) : q.kind === 'mystery' ? t('questMystery') : q.kind === 'detour' ? t('questDetour') : t('questPlayN').replace('{n}', String(q.target));
  const sub = (q: Quest) => t(q.kind === 'mode' ? 'questSubMode' : q.kind === 'mystery' ? 'questSubMystery' : q.kind === 'detour' ? 'questSubDetour' : 'questSubGames');
  ...
  return <section className={'daily-quests quests-v3' + (done ? ' is-complete' : '') + (compact ? ' is-compact' : '') + (art ? ' has-art' : '')} aria-labelledby={titleId}>
    ...
    <ul className="quest-list">{quests.map(q => {
      const body = <>
        <span className={'quest-icon quest-icon-' + q.kind} aria-hidden="true">{q.done ? <Check size={20} strokeWidth={2.8}/> : q.kind === 'games' ? <Flame size={22} strokeWidth={2.2}/> : <GameIcon mode={q.kind === 'mode' ? q.mode! : q.kind === 'detour' ? 'daily' : q.kind}/>}</span>
