...
export function DailyLoop({ app }: { app: any }) {
  ...
  useEffect(() => { let active = true; api('/puzzles/today').then(s => { if (active) setToday(s); }).catch(() => {}); return () => { active = false; }; }, [boot.stats.dailyStreak, boot.stats.dailyCount]);
  ...
  async function launch(mode: DailyMode) {
    ...
    try {
      if (mode === 'daily') await start({ ...DEFAULT_SETTINGS, mode: 'daily' });
      else { const game = await post(mode === 'rank' ? '/ranks' : '/puzzles', { mode, daily: true }); go((mode === 'rank' ? '/rank/' : '/puzzle/') + game.id); }
