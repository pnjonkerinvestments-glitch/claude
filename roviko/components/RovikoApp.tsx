...
export default function RovikoApp({ initialPath = '/' }: {
 ...
 <main id="main" className={'site-main ' + (gamePath ? 'game-main' : '')}>{fatal ? <Empty title={t('connectionError')} copy={t(errorMessage(fatal))}><button className="btn primary" onClick={refresh}>{t('retry')}</button></Empty> : !bootLoaded && ['/game/', '/puzzle/', '/rank/', '/room/', '/profile', '/friends', '/admin', '/leaderboard'].some(v => path.startsWith(v)) ? <Loading /> : path === '/' ? <HomePage /> : path === '/scoring' ? <ScoringPage /> : path === '/multiplayer' ? <MultiplayerLanding /> : path.startsWith('/room/') ? <RoomScreen key={path} code={path.split('/')[2]}/> : path.startsWith('/game/') ? <SoloScreen key={path} id={path.split('/')[2]}/> : path.startsWith('/rank/') ? <RankGame key={path} id={path.split('/')[2]} app={ctx}/> : path.startsWith('/puzzle/') ? <PuzzleGame key={path} id={path.split('/')[2]} app={ctx}/> : path === '/daily' ? <AllGames /> : path === '/duel' ? <DuelScreen /> : path === '/how-to-play' ? <HowToScreen /> : path === '/leaderboard' ? <Rankings /> : path === '/profile' ? <Profile /> : path === '/friends' ? <Friends /> : path === '/explore' ? <Explore /> : path === '/admin' ? <Admin /> : ['/privacy', '/terms', '/sources'].includes(path) ? <InfoPage kind={path.slice(1)}/> : <SEOPage slug={path.slice(1)}/>}</main>
