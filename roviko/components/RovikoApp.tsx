...
import { PuzzleDeck } from './puzzles/PuzzleDeck';
...
function GameSettings({ value, onChange, multiplayer = false, disabled = false }: any) { const { t } = useApp(); const set = (k: string, v: any) => onChange({ ...value, [k]: v }); return <div className="settings-grid">{multiplayer && <Choice label={t('play')} value={value.mode} disabled={disabled} onChange={v => set('mode', v)} options={['mixed', ...MODES].map(v => ({ value: v, label: t(v) }))}/>}<Choice label={t('rounds')} value={value.count} disabled={disabled} onChange={v => set('count', +v)} options={[5, 10, 15, 20].map(v => ({ value: v, label: v + ' ' + t('questions') }))}/>{multiplayer && <Choice label={t('timer')} value={value.timer} disabled={disabled} onChange={v => set('timer', +v)} options={[5, 10, 15, 30, 0].map(v => ({ value: v, label: v ? v + ' sec' : t('unlimited') }))}/>}<Choice label={t('difficulty')} value={value.difficulty} disabled={disabled} onChange={v => set('difficulty', v)} options={['easy', 'medium', 'hard', 'mixed'].map(v => ({ value: v, label: t(v) }))}/><Choice label={t('region')} value={value.region} disabled={disabled} onChange={v => set('region', v)} options={REGIONS.map(v => ({ value: v, label: t(v) }))}/>{!multiplayer && value.mode === 'capitals' && <div className="switch-field"><div><label htmlFor="typed-choice">{t('typed')}</label><p>{t('typedHelp')}</p></div><Switch id="typed-choice" checked={!!value.typed} onCheckedChange={v => set('typed', v)}/></div>}</div>; }
...
export default function RovikoApp({ initialPath = '/' }: {
    ...
    const start = async (settings: any, practice = false) => { if (starting.current) return; starting.current=true; setBusy(true); try {
        if (!boot.user.id)
            await refresh();
        const game = await post('/games', { settings: { ...settings, timer: 0 }, practice });
        go('/game/' + game.id);
    }
    catch (e) {
        fail(e);
...
function DailyWelcome() { const { t } = useApp(); return <div className="atelier-intro daily-welcome"><div><h1>{t('atelierTitle')}</h1><p>{t('atelierIntro')}</p></div><span className="welcome-note"><Globe2 size={18}/>{t('soloPace')}</span></div>; }
function Home() {
...
function MultiplayerLanding() { const { t, setModal } = useApp(); return <div className="multiplayer-page"><div className="page-heading centered"><span className="eyebrow"><Users size={16}/>{t('multiplayer')}</span><h1>{t('roomTitle')}</h1><p>{t('roomCopy')}</p></div><div className="multiplayer-options"><section className="panel create-panel"><div className="avatar-stack big"><Avatar id={0}/><Avatar id={1}/><Avatar id={4}/></div><h2>{t('createRoom')}</h2><p>{t('lobbyCopy')}</p><button className="btn primary wide" onClick={() => setModal('room')}><Plus size={20}/>{t('createRoom')}</button></section><section className="panel join-panel"><span className="big-icon"><LinkIcon size={34}/></span><h2>{t('joinRoom')}</h2><p>{t('joinHelp')}</p><JoinForm /></section></div><p className="center-note"><ShieldCheck size={16}/>{t('guestNote')}</p></div>; }
...
function SoloScreen({ id }: { id: string }) {
    ...
    useEffect(load, [load]);
    useEffect(() => { if (game) document.title = t(game.daily ? 'dailyTitle' : game.settings.mode) + ' | ' + BRAND.name; }, [game?.settings?.mode, game?.daily, locale]);
...
function RoomScreen({ code }: {
    ...
    const [room, setRoom] = useState<any>(null), [connected, setConnected] = useState(false), [error, setError] = useState(''), [offset, setOffset] = useState(0), [pending, setPending] = useState(false);
...
function Explore() { const { t, locale } = useApp(); const [data, setData] = useState<any[]>([]), [query, setQuery] = useState(''), [region, setRegion] = useState('World'), [selected, setSelected] = useState<any>(null), [error, setError] = useState(false), [reload, setReload] = useState(0); useEffect(() => { setError(false); fetch('/data/countries.json', { signal: AbortSignal.timeout(10000) }).then(r => r.json()).then(setData).catch(() => setError(true)); }, [reload]); const visible = data.filter(c => (region === 'World' || c.region === region) && [c.name, c.nl, ...c.capitals].join(' ').toLowerCase().includes(query.toLowerCase())); return <div className="explore-page"><div className="page-heading"><span className="eyebrow"><Globe2 size={16}/>{t('explore')}</span><h1>{t('exploreTitle')}</h1><p>{t('exploreSub')}</p></div><div className="explore-filters"><label className="search-box"><Search size={19}/><input aria-label={t('searchCountries')} placeholder={t('searchCountries')} value={query} onChange={e => setQuery(e.target.value)}/></label><Choice label={t('region')} value={region} onChange={setRegion} options={REGIONS.map(v => ({ value: v, label: t(v) }))}/></div>{error ? <Empty title={t('connectionError')}><button className="btn secondary" onClick={() => setReload(n => n+1)}>{t('retry')}</button></Empty> : !data.length ? <Loading /> : visible.length ? <div className="country-grid">{visible.map(c => <button className="country-card" key={c.id} onClick={() => setSelected(c)}><img src={c.flag} alt="" loading="lazy"/><div><strong>{locale === 'nl' ? c.nl : c.name}</strong><span>{c.capitals.join(' / ')}</span></div><ArrowUpRight size={17}/></button>)}</div> : <Empty title={t('exploreEmpty')}/>}<A href="/sources" className="text-link data-notice">{t('dataNotice')}<ArrowUpRight size={15}/></A><Dialog open={!!selected} onOpenChange={v => !v && setSelected(null)}><DialogContent className="app-modal country-modal">{selected && <><img src={selected.flag} alt="" className="country-detail-flag"/><DialogTitle className="modal-title">{locale === 'nl' ? selected.nl : selected.name}</DialogTitle><DialogDescription>{selected.official}</DialogDescription><dl className="country-facts">{[['capital', selected.capitals.join(' / ')], ['region', t(selected.region)], ['area', formatScore(selected.area) + ' km²'], ['currency', selected.currencies.join(', ')], ['languages', selected.languages.join(', ')], ['landBorders', selected.borders.map((id: string) => { const c = data.find(c => c.id === id); return c ? (locale === 'nl' ? c.nl : c.name) : id; }).join(', ') || t('noBorder')]].map(([k, v]) => <div key={k}><dt>{t(k)}</dt><dd>{v}</dd></div>)}</dl><A href="/sources" className="text-link">{t('sources')}<ArrowUpRight size={15}/></A></>}</DialogContent></Dialog></div>; }
