...
export function PuzzleGame({ id, app }: { id: string; app: any }) {
  ...
  const [picked, setPicked] = useState<{ round: number; id: string } | null>(null);
  ...
  const load = useCallback(async () => { setError(''); pending.current = null; try { const g = await api('/puzzles/' + id); latest.current = g; setGame(g); const last=g.answers.at(-1); if(g.hintSelection){setSelected(g.hintSelection);setChecked(null);setNotice({key:'puzzleHintCopy'});} else if(g.mode==='mosaic' && g.phase!=='finished' && last && !last.correct){setSelected(last.value);setChecked(g.competition?g.mosaicReview??null:reviewMosaic(g.board,last.value));setNotice({key:'puzzleTryMix'});} else {setSelected(prev => prev.filter(id => g.board?.tiles.some((tile: Tile) => tile.id === id && !g.solved.includes(tile.countryId))));} } catch { setError('puzzleLoadError'); } }, [id]);
  useEffect(() => { load(); }, [load]);
  ...
  function chooseCountry(value: string) {
    ...
    setPicked({ round: game.round, id: value });
    if(game.competition){save('answer',value)?.then(g=>{if(!muted)sound(g.answers.at(-1)?.correct?'correct':'incorrect');}).catch(()=>{});return;}
  ...
  function connect() {
    ...
    if(game.competition){save('answer',selected)?.then(g=>{const a=g.answers.at(-1)!;setChecked(g.mosaicReview??null);setNotice({key:a.correct?'puzzleConnected':'puzzleTryMix',good:a.correct});if(a.correct)setSelected([]);if(!muted)sound(a.correct?'correct':'incorrect');}).catch(()=>{});return;}
    ...
    setNotice({ key: match.correct ? 'puzzleConnected' : match.closest === game.board.size - 1 ? 'puzzleOneAway' : 'puzzleTryMix', good: match.correct, value: match.correct ? game.board.countries.find(c => c.id === match.countryId)!.name[locale as 'en' | 'nl' | 'es'] : undefined });
  ...
  return <div className={'puzzle-game puzzle-' + game.mode}>
    <GameHeader mode={game.mode} title={t(game.mode)} edition={editionLabel(game.daily, locale, t('puzzlePractice'))} count={game.mode === 'compare' ? Math.min(game.total, game.round + 1) + ' / ' + game.total : game.solved.length + ' / 4'} unit={t(game.mode === 'compare' ? 'questions' : 'countries')} progress={game.phase === 'finished' ? 1 : game.mode === 'compare' ? (game.round + +revealed) / game.total : game.solved.length / 4} onExit={() => backToStart ? backToStart() : go(game.daily ? '/daily' : '/')} exitLabel={t('back')} help={<HowToPlayButton mode={game.mode} t={t} locale={locale} auto={game.phase !== 'finished'}/>}/>
    {game.competition && game.phase!=='finished' && <DailyScoreRule mode={game.mode} t={t} score={game.score}/>}
    ...
    {game.phase === 'finished' ? <section className={'puzzle-finished' + (game.competition ? ' is-daily' : '')}><Mascot mood={game.mode === 'mosaic' || correct === game.total ? 'cheer' : correct >= game.total / 2 ? 'happy' : 'wink'} size={132} className="result-mascot"/><h1>{t(game.mode === 'mosaic' ? 'puzzleBoardComplete' : 'puzzleComparisonComplete')}</h1><p>{t('puzzleFinishedCopy')}</p><div className="puzzle-result-facts"><div><strong>{game.mode === 'compare' ? correct + '/' + game.total : '4/4'}</strong><span>{t(game.mode === 'compare' ? 'correctAnswers' : 'puzzleCountriesConnected')}</span></div><div><strong>{game.mode === 'mosaic' ? game.answers.length : game.bestStreak}</strong><span>{t(game.mode === 'mosaic' ? 'puzzleAttempts' : 'bestStreak')}</span></div></div>{game.mode === 'compare' && <div className="puzzle-answer-trail" aria-label={t('correctAnswers')}>{game.answers.map((a, i) => <span key={i} className={a.correct ? 'correct' : ''} aria-label={`${i + 1}: ${t(a.correct ? 'correct' : 'incorrect')}`}>{a.correct ? <Check size={20}/> : <X size={20}/>}</span>)}</div>}{board && <div className="mosaic-completed-countries">{board.countries.map(c => <span key={c.id}><img src={c.flag} alt=""/>{c.name[locale as 'en'|'nl'|'es']}</span>)}</div>}{game.competition ? <DailyResult app={app} date={game.daily!} mode={game.mode}/> : <NextDiscovery result={game} t={t} go={go} fail={fail ?? (()=>setError('puzzleLoadError'))} disabled={saving || moving || !!error}/>}<div className="puzzle-results-actions"><button className="btn secondary" onClick={() => backToStart ? backToStart() : go('/')}>{t('finishForNow')}</button><button className="btn ghost" disabled={saving || !!error} onClick={share}><Share2 size={18}/>{t('share')}</button><button className="btn ghost" disabled={saving || moving || !!error} onClick={again}>{t('playAgain')}</button></div>{saveState}<details className="puzzle-review"><summary>{t('puzzleReview')}</summary>{game.review?.map(q => <div className="comparison-review" key={q.id}><span>{q.topic.emoji} {q.topic.label[locale as 'en'|'nl'|'es']}</span>{q.countries.map(c => <div key={c.id}><img src={c.flag} alt=""/><strong>{c.name[locale as 'en'|'nl'|'es']}</strong><span>{formatMetric(c.value, q.topic.unit, locale as 'en'|'nl'|'es')}</span>{c.id === q.correct && <Check size={17}/>}</div>)}</div>)}{board?.countries.map(c => <div className="mosaic-review-country" key={c.id}><h3><img src={c.flag} alt=""/>{c.name[locale as 'en'|'nl'|'es']}</h3><div>{board.tiles.filter(tile => tile.countryId === c.id && tile.kind !== 'name' && tile.kind !== 'flag').map(tile => <span key={tile.id}><small>{t('tile' + tile.kind.charAt(0).toUpperCase() + tile.kind.slice(1))}</small><TileContent tile={tile} locale={locale as 'en'|'nl'|'es'} t={t}/></span>)}</div><MosaicFactDetails tile={board.tiles.find(tile => tile.countryId === c.id && tile.kind === 'fact')} locale={locale as 'en'|'nl'|'es'} t={t}/></div>)}</details></section> : game.mode === 'compare' && question ? <>
      <div className="comparison-heading"><span className="topic-pill"><span aria-hidden="true">{question.topic.emoji}</span>{question.topic.label[locale as 'en'|'nl'|'es']}{question.referenceYear && <small>{question.referenceYear}</small>}</span><h1>{question.topic.prompt[locale as 'en'|'nl'|'es']}</h1><p>{t(game.practice ? 'reviewRoundCopy' : 'puzzleCarryCopy')}</p><details className="game-rule-help"><summary>{t('gameRules')}</summary><p>{t('comparisonTieRule')}</p></details><label className="auto-next"><input type="checkbox" checked={autoNext} onChange={e => (setAutoNext(e.target.checked), writePreference('rv_auto_next', e.target.checked ? 'on' : 'off'))}/>{t('autoNextAll')}</label>{autoNext && <p className="auto-explanation">{t('autoNextExplain')}</p>}</div>
      <div className="comparison-cards">{question.countries.map((country, i) => { const chosen = revealed && game.answers.at(-1)?.value === country.id, winner = revealed && question.correct === country.id; return <button ref={i === 0 ? focusTarget : undefined} className={'comparison-country ' + (i === 1 && question.carried ? 'carried ' : 'fresh ') + (winner ? 'winner' : chosen ? 'wrong' : '') + (chosen || (!revealed && picked?.round === game.round && picked.id === country.id) ? ' is-picked' : '')} key={country.id} onClick={() => chooseCountry(country.id)} disabled={revealed || !!error || saving} aria-label={country.name[locale as 'en'|'nl'|'es']}><span className="country-key" aria-hidden="true">{t(i === 1 && question.carried ? 'puzzleCarried' : 'puzzleNew')}</span><img src={country.flag} alt=""/><h2>{country.name[locale as 'en'|'nl'|'es']}</h2><div className="comparison-value">{revealed ? <><strong>{formatMetric(country.value, question.topic.unit, locale as 'en'|'nl'|'es')}</strong><span>{winner ? <><Check size={17}/>{t('puzzleHigher')}</> : <>{chosen && <X size={17}/>} {t('puzzleLower')}</>}</span></> : i === 1 && question.carried && Number.isFinite(country.value) ? <><strong>{formatMetric(country.value, question.topic.unit, locale as 'en'|'nl'|'es')}</strong><span>{t('puzzleCarried')}</span></> : <span>{t('puzzleChooseThis')}<ArrowRight size={17}/></span>}</div>{chosen && <span className="your-choice">{t('puzzleYourChoice')}</span>}</button>; })}<span className="comparison-or">{t('puzzleOr')}</span></div>
      {revealed && <div className={'comparison-reveal ' + (game.answers.at(-1)?.correct ? 'good' : 'bad')} role="status"><span aria-hidden="true">{game.answers.at(-1)?.correct ? '🎉' : '✕'}</span><div><strong>{t(game.answers.at(-1)?.correct ? 'puzzleCorrect' : 'puzzleLearned')}</strong><p>{question.countries.find(c => c.id === question.correct)!.name[locale as 'en'|'nl'|'es']} — <strong>{formatMetric(question.countries.find(c => c.id === question.correct)!.value, question.topic.unit, locale as 'en'|'nl'|'es')}</strong> {t('comparisonGreater')} {question.countries.find(c => c.id !== question.correct)!.name[locale as 'en'|'nl'|'es']} — <strong>{formatMetric(question.countries.find(c => c.id !== question.correct)!.value, question.topic.unit, locale as 'en'|'nl'|'es')}</strong>.</p><p className="comparison-detail">{question.topic.explanation[locale as 'en'|'nl'|'es']}</p></div></div>}
      ...
      <div className="mosaic-heading"><h1>{t('puzzleMatchHeading')}</h1><p>{t('puzzleMatchInstruction').replace('{n}', String(board.size))}</p>{board.size > 3 && <p className="mosaic-edition-line"><span aria-hidden="true">↻</span>{t('mosaicDailyFacts')}{board.factDate && <time dateTime={board.factDate}>{new Intl.DateTimeFormat(locale === 'es' ? 'es-ES' : locale === 'nl' ? 'nl-NL' : 'en-GB', { day: 'numeric', month: 'short', timeZone: 'UTC' }).format(new Date(board.factDate + 'T00:00:00Z'))}</time>}</p>}</div>
      <div className="mosaic-selection mosaic-overview"><div className="selection-slots">{(['flag','name','shape','fact','capital'] as const).filter(kind => board.tiles.some(tile => tile.kind === kind)).map(kind => { const tile = board.tiles.find(tile => tile.kind === kind && selected.includes(tile.id)); return <div key={kind} className={tile ? 'filled' : ''}><small>{t('tile' + kind.charAt(0).toUpperCase() + kind.slice(1))}</small>{tile ? <TileContent tile={tile} locale={locale as 'en'|'nl'|'es'} t={t}/> : <span aria-hidden="true">＋</span>}</div>; })}</div></div>
      <div className="mosaic-solved" aria-live="polite">{game.solved.map((id, i) => { const c = board.countries.find(c => c.id === id)!; return <div className={'mosaic-group group-' + i} key={id}><img src={c.flag} alt=""/><div className="mosaic-group-text"><strong>{c.name[locale as 'en'|'nl'|'es']}</strong><small>{groupSummary(board.tiles.filter(tile => tile.countryId === id), locale as 'en'|'nl'|'es', t)}</small></div><Check size={20}/><MosaicFactDetails tile={board.tiles.find(tile => tile.countryId === id && tile.kind === 'fact')} locale={locale as 'en'|'nl'|'es'} t={t}/></div>; })}</div>
      <div className="mosaic-board" data-clues={board.size} aria-label={t('mosaic')}>{displayTiles.map(tile => { const verdict = checked?.tiles.find(item => item.id === tile.id); const solved = game.solved.includes(tile.countryId); return <button key={tile.id} className={'mosaic-tile kind-' + tile.kind + (solved ? ' solved' : verdict ? verdict.correct ? ' clue-correct' : ' clue-wrong' : selected.includes(tile.id) ? ' selected' : '')} aria-pressed={selected.includes(tile.id)} aria-label={t('tile' + tile.kind.charAt(0).toUpperCase() + tile.kind.slice(1)) + (tile.text ? ': ' + tile.text[locale as 'en'|'nl'|'es'] : '') + (verdict ? '. ' + (verdict.correct ? t('mosaicFits') : (game.competition?t('competitionNotFit'):t('mosaicBelongsTo')+' '+board.countries.find(c => c.id === tile.countryId)!.name[locale as 'en'|'nl'|'es'])) : '')} disabled={saving || !!error || solved} onClick={() => { setSelected(prev => selectMosaicTile(board, game.solved, prev, tile.id)); setNotice(null); setChecked(null); }}><span className="tile-kind">{t('tile' + tile.kind.charAt(0).toUpperCase() + tile.kind.slice(1))}</span><TileContent tile={tile} locale={locale as 'en'|'nl'|'es'} t={t}/>{verdict && !solved && <span className="tile-verdict">{verdict.correct ? <Check size={15}/> : <X size={15}/>}<span>{verdict.correct ? t('mosaicFits') : (game.competition?t('competitionNotFit'):board.countries.find(c => c.id === tile.countryId)!.name[locale as 'en'|'nl'|'es'])}</span></span>}<span className="tile-tick" aria-hidden="true">{!verdict && (selected.includes(tile.id) || solved) ? <Check size={14}/> : null}</span></button>; })}</div>
      <div className="mosaic-dock">
        ...
        <div className={'mosaic-notice ' + (notice?.good ? 'good' : checked ? 'bad' : '')} role="status">{checked && !notice?.good ? <><X size={18}/><span>{t('mosaicMismatch').replace('{country}', board.countries.find(c => c.id === checked.countryId)!.name[locale as 'en'|'nl'|'es'])}</span></> : notice ? <><span aria-hidden="true">{notice.good ? '✓' : '💡'}</span>{t(notice.key)} {notice.value}</> : t('puzzleNoLives')}</div>
        <div className="mosaic-toolbar"><div><button className="icon-btn" aria-label={t('puzzleShuffle')} disabled={saving} onClick={() => { const ids = displayTiles.filter(t => !game.solved.includes(t.countryId)).map(t => t.id); for (let i = ids.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [ids[i], ids[j]] = [ids[j], ids[i]]; } let cursor = 0; setOrder(displayTiles.map(t => game.solved.includes(t.countryId) ? t.id : ids[cursor++])); }}><Shuffle size={19}/></button><button className="btn ghost" disabled={!selected.length || saving} onClick={() => { setSelected([]); setNotice(null); setChecked(null); }}>{t('puzzleClear')}</button><button className="icon-btn" disabled={saving||!!error||(!!game.competition&&!board.tiles.some(t=>t.kind==='name'&&selected.includes(t.id)))} aria-label={t(game.competition?'competitionHint':'puzzleHint')} title={t(game.competition?'competitionChooseAnchor':'puzzleHint')} onClick={() => { if(game.competition){save('hint',selected)?.then(g=>{setSelected(g.hintSelection??[]);setChecked(null);setNotice({key:'puzzleHintCopy'});}).catch(()=>{});return;} setSelected(prev => mosaicHint(board, game.solved, prev)); setChecked(null); setNotice({ key: 'puzzleHintCopy' }); }}><Lightbulb size={19}/>{game.competition&&<small>−125</small>}</button></div><button className="btn primary mosaic-check" disabled={selected.length !== board.size || saving || !!error} onClick={connect}>{t('puzzleConnect')}<span className="selection-count">{selected.length}/{board.size}</span></button></div>
      </div>
...
/** One line under a solved country: which pieces belonged to it, with the fact spelled out. */
function groupSummary(tiles: Tile[], locale: 'en' | 'nl' | 'es', t: (key: string) => string) {
  return tiles.map(tile => { const kind = t('tile' + tile.kind.charAt(0).toUpperCase() + tile.kind.slice(1)); const stat = tile.fact?.stat; return stat ? stat.label[locale] + ': ' + stat.value[locale] + (stat.unit[locale] ? ' ' + stat.unit[locale] : '') : kind; }).join(' · ');
}

export function TileContent({ tile, locale, t }: { tile: Tile; locale: 'en' | 'nl' | 'es'; t: (key: string) => string }) {
  ...
  const text = tile.text?.[locale] ?? '', longest = Math.max(0, ...text.split(/\s+/).map(w => w.length));
  // Long words get a smaller size instead of breaking mid-word on a narrow tile.
  return <span className={'tile-text' + (longest > 9 || text.length > 18 ? ' is-long' : '') + (longest > 12 || text.length > 26 ? ' is-longer' : '')}>{text}</span>;
}
