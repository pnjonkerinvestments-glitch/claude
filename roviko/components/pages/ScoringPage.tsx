...
import { ArrowRight, Crown, Flame, ShieldCheck, Sparkles, Timer, Users, RotateCcw } from 'lucide-react';
...
export function ScoringPage() {
  ...
  return <div className="page scoring-page">
    ...
    <ul className="fact-tiles">
      ...
      <li><Timer size={22} aria-hidden="true"/><strong>0 s</strong><span>{t('scoringNoTimer')}</span></li>
    </ul>
    <div className="points-bar" aria-hidden="true">{DAILY_MODES.map(m => <span key={m} className={'points-seg seg-' + m}><GameIcon mode={m} size="sm"/></span>)}</div>
    ...
    <section className="page-section" aria-labelledby="scoring-more">
      ...
      <div className="rule-grid">
        <article className="rule-card"><span className="rule-icon" aria-hidden="true"><Flame size={22}/></span><div><h3>{t('scoringStreak')}</h3><p>{t('scoringStreakCopy')}</p><p className="rule-aside"><ShieldCheck size={15} aria-hidden="true"/>{t('freezeExplain')}</p></div></article>
