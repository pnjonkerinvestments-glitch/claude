...
- Room membership, current match/round identity and host checks on the server. Multiplayer solutions are omitted until reveal. Solo has a separate current-round solution for immediate learning feedback; it awards no points or XP and is excluded from competitive rankings. Opaque flag asset identifiers prevent the straightforward filename answer leak.
