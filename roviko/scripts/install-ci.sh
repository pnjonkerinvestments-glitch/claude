...
NODE
  ...
  cached_vinext="$(node --input-type=module - "${expected_cache}" "${locked_integrity}" <<'NODE'
import {existsSync} from 'node:fs';
const [algorithm, digest] = process.argv[3].split('-');
const hex = Buffer.from(digest, 'base64').toString('hex');
const path = `${process.argv[2]}/_cacache/content-v2/${algorithm}/${hex.slice(0,2)}/${hex.slice(2,4)}/${hex.slice(4)}`;
if(existsSync(path)) process.stdout.write(path);
NODE
)"
  if [[ -n "${cached_vinext}" ]]; then
    echo "[sites] verifying cached locked vinext tarball"
    cp "${cached_vinext}" "${preflight_tarball}"
  else
