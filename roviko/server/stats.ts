...
export async function stats(env: Env, id: string) {
    ...
    let ref = new Date(today + 'T00:00:00Z').getTime();
    if (dates[0]?.date !== today)
        ref -= 86400000;
    for (const row of dates) {
        if (row.date === new Date(ref).toISOString().slice(0, 10)) {
            dailyStreak++;
            ref -= 86400000;
        }
        else
    ...
    return { ...s, reviews, stamps, discovered: stamps.length, accuracy: s.total ? Math.round(s.correct / s.total * 100) : 0, averageTime: s.total ? Math.round(s.duration / s.total) : 0, level: 1 + Math.floor(Math.sqrt(s.xp / 100)), levelProgress: Math.round((Math.sqrt(s.xp / 100) % 1) * 100), dailyStreak, dailyCount: dates.length, dailyDone: dates.some((d: any) => d.date === today), modes, achievements: unlocked.map(a => a.id), rating: 1000, recent: await rows(env, 'SELECT * FROM game_results WHERE user_id=? ORDER BY created_at DESC LIMIT 12', id), weak: await rows(env, 'SELECT country_id,mode,correct,attempts FROM concept_performance WHERE user_id=? AND correct<attempts ORDER BY (1.0*correct/attempts),attempts DESC LIMIT 16', id) };
}
