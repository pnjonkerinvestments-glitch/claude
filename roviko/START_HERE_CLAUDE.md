...
- Bonustour (1.21): na de zes spellen met punten staan de zes klassieke spellen (vlaggen, hoofdsteden, kaart, buren, grootte, Clue Trail) klaar als editie van de dag: tien vragen, dezelfde landen voor iedereen op een UTC-datum, één opgeslagen spel per speler (`startBonus` in `server/solo.ts`, sessies `bonus:<mode>`, `lib/bonus.ts`). Geen ranglijstpunten (de regel hierboven blijft); wel "beter dan X% van de spelers van vandaag" (`/api/bonus/standing`). Opnieuw spelen daarna is gewoon willekeurig oefenen.
- Rondleiding (1.21): bij het eerste bezoek een korte uitleg in 4 stappen, voor gasten met een vijfde stap over een gratis account (`components/home/WelcomeTour.tsx`, localStorage `roviko:tour`); ook te openen via menu → Rondleiding.
