...
test('same UTC daily for two guests, twenty complete rounds and immutable first result',async()=>{const a=await bootstrap(),b=await bootstrap();const sa=await request(a.cookie,'/games','POST',{settings:{...settings,mode:'daily'}}),sb=await request(b.cookie,'/games','POST',{settings:{...settings,mode:'daily'}});assert.deepEqual(sa.data.question,sb.data.question);let g=sa.data;assert.equal(g.total,20);for(let i=0;i<20;i++){const row=await db.prepare('SELECT state FROM game_sessions WHERE id=?').bind(g.id).first();const state=JSON.parse(row.state);state.startAt=Date.now()-1000;await db.prepare('UPDATE game_sessions SET state=? WHERE id=?').bind(JSON.stringify(state),g.id).run();assert.equal((await request(a.cookie,'/games/'+g.id+'/answer','POST',{round:i,answer:state.questions[i].correct})).status,200);g=(await request(a.cookie,'/games/'+g.id+'/next','POST',{})).data;}assert.equal(g.phase,'finished');assert.equal(g.answers.length,20);const again=await request(a.cookie,'/games','POST',{settings:{...settings,mode:'daily'}});assert.equal(again.data.id,g.id);assert.equal(again.data.score,g.score);assert.equal((await db.prepare('SELECT COUNT(*) count FROM daily_challenge_results WHERE user_id=?').bind(a.data.user.id).first()).count,1);});
...
test('logging into an existing account merges guest sessions once, preserves its first daily, and revokes guest bearer sessions',async()=>{
 ...
 let stats=(await request(login.cookie,'/profile')).data.stats;assert.equal(stats.games,2);assert.equal(stats.total,40);assert.equal(stats.correct,40);assert.equal(stats.dailyCount,1);
...
test('Daily Detour uses accuracy points for maps, hides solutions and ignores client scores',async()=>{
 ...
 while(g.phase!=='finished'){
  ...
  const feedback=saved.data.feedback;assert.equal(feedback.points,q.mode==='pinpoint'?Math.round(feedback.mapPoints/20):50);expected+=feedback.points;
  g=(await request(a.cookie,'/games/'+g.id+'/next','POST',{})).data;
 }
...
test('the next flag can load while the answer is shown, but never early and never a hidden Clue Trail flag',async()=>{
 ...
 // The Daily Detour starts with a random question type; its flags, when present, are always opaque game assets.
 if(daily.question.flagUrl!==undefined) assert.match(daily.question.flagUrl,/^\/api\/game-asset\//);
