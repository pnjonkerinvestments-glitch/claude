...
export type PuzzleState = { hintSelection?:string[]; competition?: import('../daily-scoring').Competition; mosaicAid?: Record<string,{wrong:boolean;hints:number}>; practice?: boolean; reviewOf?: string; datasetVersion?: string; id: string; mode: PuzzleMode; daily: string | null; phase: 'question' | 'reveal' | 'finished'; round: number; startedAt: number; turnAt: number; settings: { topic: string; size: 3 | 4 | 5 }; questions: CompareRound[]; board: MosaicBoard | null; answers: Attempt[]; solved: string[]; streak: number; bestStreak: number };
export type PuzzleView = Omit<PuzzleState, 'questions' | 'startedAt' | 'turnAt'> & { question: CompareRound | null; review?: CompareRound[]; total: number; version: number; score?: number; mosaicReview?: ReturnType<typeof reviewMosaic>; hintSelection?: string[]; learning: true };
