...
export function dailyRoundPoints(mode: PointMode, answer: any) {
  if (mode === 'trail') return trailPoints(answer.correct, answer.cluesUsed);
  if (mode === 'daily') return answer.mode === 'pinpoint' ? Math.round(Math.max(0, Math.min(1000, answer.mapPoints ?? 0)) / 5) : answer.correct ? 200 : 0;
  if (mode === 'compare') return answer.correct ? 100 : 0;
  return 0;
}
export function dailyScore(s: { competition?: Competition; answers: any[]; mosaicAid?: Record<string, { wrong: boolean; hints: number }> }) {
  ...
  } else score = s.answers.reduce((sum, a) => sum + dailyRoundPoints(mode, a), 0);
